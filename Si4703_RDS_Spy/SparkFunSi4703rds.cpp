...
// Based on SparkFun Si4703 Arduino Library
// https://github.com/sparkfun/SparkFun_Si4703_Arduino_Library/tree/V_1.2.0
//
...
/*void Si4703_Breakout::readRDSAndSend()
{ 
    readRegisters();
    if(getRegister(STATUSRSSI) & (1<<RDSR)) {

      String blockA =  String(si4703_registers[RDSA], HEX), blockB =  String(si4703_registers[RDSB], HEX), blockC =  String(si4703_registers[RDSC], HEX), blockD =  String(si4703_registers[RDSD], HEX);
      blockA.toUpperCase();
...
uint16_t Si4703_Breakout::getRegister(int index) {
  return si4703_registers[index];
} 
